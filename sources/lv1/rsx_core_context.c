...
* 
***********************************************************************/
int32_t rsx_core_context_iomap(rsx_ctx_obj_t *rsx_ctx, uint64_t ea, int64_t lm_offset, int32_t size, uint64_t flags)
{
	int32_t ret = -1, page_size = rsx_ctx->page_size, done = 0;
...
* 
***********************************************************************/
int32_t rsx_core_context_get_size_of_reports()
{
...
* 
***********************************************************************/
static rsx_ctx_dma_obj_t *rsx_core_context_get_dma_object_by_index(rsx_ctx_obj_t *rsx_ctx, int32_t idx)
{
...
* 
***********************************************************************/
static int32_t rsx_core_context_2146F4(rsx_ctx_obj_t *rsx_ctx)
{
	int32_t i, ret = -1, value;
	uint32_t class;
  ...
  if(core == NULL)
	...
	DRVI_write32(rsx_ctx->idx, (int32_t)rsx_bus_ioif0_get_chip_revision((void*)ioif0), 4);
	...
	ch_obj = (void*)rsx_ctx->ch_obj;
	DRVI_write32(rsx_ctx->idx, (int32_t)rsx_object_channel_get_id((void*)ch_obj), 0xC);
	
	
	
	// store NV clock frequency(0x1DCD6500(500 MHz)) into driver info 0x0010:
	clock = (void*)core->dev_clock_1_obj;
	DRVI_write32(rsx_ctx->idx, (int32_t)rsx_device_clock_rsx_get_frequency((void*)clock), 0x10);
	
	// store display clock frequency(0x26BE3680(650 MHz)) into driver info 0x0014:
	clock = (void*)core->dev_clock_5_obj;
	DRVI_write32(rsx_ctx->idx, (int32_t)rsx_device_clock_rsx_get_frequency((void*)clock), 0x14);
	
	
	// ? memory related, TODO: something wrong with stored values!
	DRVI_write32(rsx_ctx->idx, (int32_t)count_set_bits(mem_ctx->unk_40), 0x18);
	DRVI_write32(rsx_ctx->idx, (int32_t)count_set_bits(mem_ctx->unk_48), 0x1C);
	DRVI_write32(rsx_ctx->idx, (int32_t)(mem_ctx->unk_2C - mem_ctx->unk_28), 0x20);
	DRVI_write32(rsx_ctx->idx, (int32_t)(mem_ctx->unk_38 - mem_ctx->unk_30), 0x24);
  ...
  value = read_BAR0(0x28000100200);
	...
	// ? reports offsets
	DRVI_write32(rsx_ctx->idx, (int32_t)(rsx_ctx->unk_0E0 - rsx_ctx->reports_addr), 0x2C);  // ctx 0: offset 0x1000
	DRVI_write32(rsx_ctx->idx, (int32_t)(rsx_ctx->unk_0E8 - rsx_ctx->reports_addr), 0x30);  // ctx 0: offset 0
	DRVI_write32(rsx_ctx->idx, (int32_t)(rsx_ctx->unk_0F8 - rsx_ctx->reports_addr), 0x34);  // ctx 0: offset 0x1400
	
	// store system mode flags into driver info 0x0054:
	DRVI_write32(rsx_ctx->idx, (int32_t)(rsx_ctx->sys_mode), 0x54);
	...
	// ?  
	DRVI_write32(rsx_ctx->idx, (int32_t)(rsx_ctx->unk_020), 0x12B8);  // ctx 0(0x38), ctx 1(0x39)
	
	// store context IO offset into driver info 0x2B60:
	DRVI_write32(rsx_ctx->idx, (int32_t)rsx_ctx->io_offset, 0x2B60);
	
	// ? IO start address?
	value = core_mem->bar_1_addr + (mem_ctx->lm_start_lpar - core_mem->ddr_lpar_addr);
	DRVI_write32(rsx_ctx->idx, (int32_t)(value - rsx_bus_ioif0_get_BAR1_addr((void*)ioif0)), 0x2B5C);
	
	// BAR0
	value = read_BAR0(0x28000001540);
	value = (value & 0x1FF00) >>8;          // value[15:23], unk
	DRVI_write32(rsx_ctx->idx, (int32_t)value, 0x2B64);
...
* 
***********************************************************************/
static int32_t rsx_core_context_214C58(rsx_ctx_obj_t *rsx_ctx)
{
	int32_t ret = -1, sh = 0;
	int64_t channel_id;
	rsx_dev_core_obj_t *core = NULL;
  ...
  if(core == NULL)
	...
	for(sh = 63; sh > -1; sh--)
		if(rsx_ctx->page_size & ((uint64_t)1<<sh))
		  break;
...
* 
***********************************************************************/
static int32_t rsx_core_context_2143E0(rsx_ctx_obj_t *rsx_ctx)
{
	int32_t i, k, ret = -1;
	rsx_dev_core_obj_t *core = NULL;
