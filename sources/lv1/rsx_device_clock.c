...
static void rsx_device_clock_rsx_21F0C0(rsx_dev_clock_obj_t *clock_obj)
{
	...
	if((clock_obj->id == 1) || (clock_obj->id == 5))
	{
		...
		if((value_1 & 0x400) >>10)
		{
			...
			clock_obj->freq = (int32_t)(((t1 * 0x5F5E100) / t2) / (1 <<t3));
			return;
		}
...
* 
***********************************************************************/
int32_t rsx_device_clock_rsx_get_frequency(rsx_dev_clock_obj_t *clock_obj)
{
...
* 
***********************************************************************/
void rsx_device_clock_rsx_create(rsx_dev_clock_obj_t *clock_obj, int32_t id, int32_t arg1)
{
	int32_t i;
